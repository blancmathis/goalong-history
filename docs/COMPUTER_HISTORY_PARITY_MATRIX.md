...
| Visible capture state | recording, paused, suppressed, permission-missing, and tap-unavailable states are visible | menu-bar and dashboard runtime presentation; persisted capture health | capture-health tests | revoke/restore Accessibility and Input Monitoring; pause/resume | implemented; live-unproven | verify wording and state changes against the signed app identity | `RuntimePresentation`, `CaptureHealthEvaluator` |
...
| Rich Context is honest | UI distinguishes event tap, Accessibility, Input Monitoring, rich context, and degraded metadata-only capture | permission/capture-health surfaces and semantic opt-in | health tests | run with each permission combination | implemented; live-unproven | audit every UI surface for overclaiming when semantic coverage is low | real UI benchmark |
...
| Granular deletion | item, ten minutes, hour, day/session and all-history deletion remove derived indexes/memories consistently after confirmation | deletion planner and existing UI/store paths | retention/deletion planner tests | execute each scope on a disposable history and rebuild | partially implemented; live-unproven | end-to-end physical deletion/rebuild coverage is incomplete | deletion integration report |
...
| Timeline coverage honesty | metadata-only periods are not presented as complete semantic understanding | coverage counts, pair ratio and limitation text | strict checker and memory consistency tests | open a large low-context day | implemented in data model; UI audit pending | timeline wording and high-volume CPU test remain open | UI/performance benchmark |
