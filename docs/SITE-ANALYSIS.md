...
The website connection sheet also prepares `goalong.rhythm-analysis.v1` from a user-selected local interval. It is a separate source-selection journey using the same isolated Codex transport. The user chooses evidence before authorizing analysis; titles/domains and optional authorized rich excerpts may contain personal data. The strict response must match the request ID and SHA-256 digest, retain every episode ID in order, reference only supplied evidence, and preserve unknown gaps. The agent cannot supply or overwrite measured durations. An unsupported provider configuration or tool invocation stops the operation.

The user can review each proposed association, edit the interpretation, and independently choose whether the website receives aggregates, episodes, context or precise start time. Another agent can work from a protected exported request and return the same pinned annotation contract. That file exchange does not give the agent filesystem access through Goalong. Neither path submits to the website until the connector's exact payload is reviewed and sent.

The 11 September change adds a synthetic stdio-provider test, contextual-evidence tests and a native journal-to-website integration check. Full Swift validation passed, and the installed Codex sandbox was exercised separately without an account. The app build was installed with the existing stable macOS identity; visual inspection of the new native screens was blocked by the locked Mac. A real authenticated generation of this new contextual contract is still a distinct acceptance check.
