...
`screen-time` returns `availableDevices` with stable IDs. Omit a scope flag for Apple’s visible **All Devices** value, use `--mac-only` for this Mac, or pass one or more comma-separated IDs through `--devices`. When the Accessibility presentation reader is available, All Devices comes directly from Apple's visible aggregate; one device and partial subsets use Apple's visible per-device rows, without cross-device deduplication. The JSON `sourceAssurance` field states whether the result is that observable Apple presentation, a public export, a private aggregate, or a reconstruction.
...
- The installed `goalong` command resolves to Goalong History's signed main executable and enters a headless read-only CLI mode before AppKit starts. macOS can still attribute protected-file access to the terminal or agent that launched a child process.
  ...
  The response first asks the running app for the values currently presented by Apple System Settings. That on-demand read runs on Goalong's main thread, uses a bounded in-memory cache, and stores no screenshot or Apple presentation snapshot. If unavailable, it tries ScreenTimeAgent’s private Apple per-device aggregate usage blocks and then exposes an explicit reconstruction state. The provenance distinguishes the observable Apple presentation, a public DeviceActivity export, a private Apple aggregate and a reconstructed Apple-usage view; private formats are never claimed as certified Settings parity.
