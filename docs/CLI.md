...
- The installed `goalong` command resolves to Goalong History's signed main executable and enters a headless read-only CLI mode before AppKit starts. macOS can still attribute protected-file access to the terminal or agent that launched a child process.
  ...
  For today, the CLI asks the already-running Goalong app through a user-only Unix socket (`0600`) to refresh the active daily record under Goalong's existing Full Disk Access decision. The app returns at most 64 MiB in memory and does not save a separate response. If the app is not running, active-day refresh fails clearly. For a completed day, the CLI reads Goalong's owner-only normalized daily record directly and never opens Apple stores.
