...
- The installed `goalong` command resolves to Goalong History's signed main executable and enters a headless read-only CLI mode before AppKit starts. macOS can still attribute protected-file access to the terminal or agent that launched a child process.
  ...
  The response asks the running app to read ScreenTimeAgent’s private Apple per-device aggregate usage blocks in place and then exposes an explicit reconstruction state when that Data Vault is unavailable. Every request rechecks the Apple source fingerprints and builds a new response; response payloads are never cached. A multi-day `ask` performs one bounded Apple range read (at most 31 days), then derives each daily summary transiently from that coherent in-memory collection instead of reopening the same databases once per day. It never opens or controls System Settings, sends synthetic input, captures a screenshot, or saves an Apple usage response. The provenance distinguishes a public DeviceActivity export, a private Apple aggregate and a reconstructed Apple-usage view; private formats are never claimed as certified Settings parity.
...
The CLI does not start a daemon, mutate Goalong settings, refresh an AI recap, or write query results. The optional Screen Time handoff reuses the already-running Goalong recorder process and a bounded local socket; it creates neither a response file nor a network listener. A direct `screen-time` command always performs an on-demand source check. `ask` recognizes Screen Time, device-usage, application-duration and productivity-duration questions and requests fresh Apple data for the relevant days through one bounded range read. A direct Screen Time-only question skips unrelated Computer History reconstruction; productivity, work-summary and other mixed questions still combine both evidence sources. A question without an explicit period reads today only; `today`, `yesterday`/`hier`, `last N days`/`N derniers jours` and explicit ranges use the interval parsed from the question. Its embedded context keeps exact totals and device totals, includes the 24 most-used applications per day, and states any day or application omissions with the exact follow-up command. A normal invocation exits after the JSON response, so it adds no persistent process or idle RAM cost.
