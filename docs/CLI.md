...
The graphical path-entry alternative was verified through CUA in an isolated native preview using the unchanged production connection view and synthetic data. Choosing the synthetic token path, pressing **Valider ce fichier**, and confirming **Protéger ce fichier** changed that file from 0644 to 0600 while retaining the current macOS owner. A preview with applications and the saved summary contained three synthetic devices and three application rows (2392 bytes); the loopback mock returned one imported day with an unverified receipt. Removing the iPad produced a refreshed two-device, two-application preview (1769 bytes). No real history or production account was used.

Known QA limitation: the macOS file picker's **Open** button remained disabled during CUA selection despite the selected token row and explicit file-selection configuration. This has not been resolved or validated manually. The path-entry alternative above completed protection, review and submission and remains available independently of that picker. Native builds and isolated preview checks do not mean the user's installed Goalong History app has been replaced or updated; no such installation was performed in this validation. All preview and mock-server processes were stopped after the checks.
