#!/bin/bash
set -euo pipefail
trap 'echo "Public installer safety test failed at line $LINENO." >&2' ERR
...
if privacy_identity_replacement_allowed \
  ...
  echo "An ad-hoc update was allowed to break permission continuity." >&2
  exit 1
fi
[[ "$PRIVACY_REAUTH_REQUIRED" -eq 0 ]]
privacy_identity_replacement_allowed 'adHoc' 'development' 'old-hash' 'stable-certificate'
[[ "$PRIVACY_REAUTH_REQUIRED" -eq 1 ]]
if privacy_identity_replacement_allowed 'development' 'adHoc' 'stable' 'new-hash'; then
  echo 'Certificate-backed installation may not be downgraded to ad-hoc.' >&2; exit 1
fi
privacy_identity_replacement_allowed 'development' 'development' 'stable' 'stable'
if privacy_identity_replacement_allowed \
...
echo "Public installer safety tests passed: stable identity preservation and ad-hoc downgrade refusal, exact identity, symlink/tamper refusal, target-filesystem staging, privacy/single-app fail-closed checks, rollback, interrupted-install recovery, and obsolete Local-edition cleanup."
